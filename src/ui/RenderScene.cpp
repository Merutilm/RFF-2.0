...
namespace merutilm::rff2 {
    ...
    void RenderScene::requestColor() {
        colorRequested = true;
    }

    void RenderScene::requestResize() {
        resizeRequested = true;
    }

    void RenderScene::requestRecompute() {
    ...
    void RenderScene::setStatusMessage(const int index, const std::string_view &message) const {
        (*statusMessageRef)[index] = std::string("  ").append(message);
    }

    Settings &RenderScene::getSettings() {
        return settings;
    }

    ParallelRenderState &RenderScene::getState() {
        return state;
    }

    MandelbrotPerturbator *RenderScene::getCurrentPerturbator() const {
        return currentPerturbator.get();
    }


    void RenderScene::setCurrentPerturbator(std::unique_ptr<MandelbrotPerturbator> perturbator) {
        this->currentPerturbator = std::move(perturbator);
    }

    ApproxTableCache &RenderScene::getApproxTableCache() {
        return approxTableCache;
    }

    BackgroundThreads &RenderScene::getBackgroundThreads() {
    ...
    bool RenderScene::isCreateImageRequested() const {
        return createImageRequested;
    }

    bool RenderScene::isResizeRequested() const {
        return resizeRequested;
    }

    bool RenderScene::isColorRequested() const {
    ...
    int RenderScene::getCHRequest() const {
        return chRequest;
    }

    void RenderScene::clientResizeRequestSolved() {
        cwRequest = 0;
        chRequest = 0;
    }
}
