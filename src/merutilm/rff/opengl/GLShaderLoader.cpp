...
void GLShaderLoader::compile(const int shader, const std::string &src) {
    ...
    if (success == GL_FALSE) {
        glGetShaderInfoLog(shader, MAX_LOG_LEN, nullptr, infoLog.data());
...
void GLShaderLoader::link() const {
    ...
    if (success == GL_FALSE) {
        glGetProgramInfoLog(shaderProgram, MAX_LOG_LEN, nullptr, infoLog.data());
        std::cout << "Error: " << infoLog << std::endl;
    }
}
...
GLShaderLoader::GLShaderLoader(std::string vertexName, std::string fragmentName) {
    {
        try {
            vertexName = SHADER_PATH_PREFIX + vertexName + VERTEX_PATH_SUFFIX;
            fragmentName = SHADER_PATH_PREFIX + fragmentName + FRAGMENT_PATH_SUFFIX;
