...
merutilm::rff::SettingsMenu::SettingsMenu(const HMENU hMenubar) : menubar(hMenubar) {
    configure();
}

merutilm::rff::SettingsMenu::~SettingsMenu() {
    DestroyMenu(menubar);
...
void merutilm::rff::SettingsMenu::configure() {
...
HMENU merutilm::rff::SettingsMenu::addChildMenu(const HMENU target, const std::string_view child) {
...
HMENU merutilm::rff::SettingsMenu::addChildItem(const HMENU target, const std::string_view child,
                                    const std::function<void
                                        (SettingsMenu &, RenderScene &)> &callback) {
    return add(target, child, callback, false, false, std::nullopt);
}

HMENU merutilm::rff::SettingsMenu::addChildCheckbox(const HMENU target, const std::string_view child,
...
HMENU merutilm::rff::SettingsMenu::add(const HMENU target, const std::string_view child,
...
void merutilm::rff::SettingsMenu::executeAction(RenderScene &scene, const int menuID) {
...
bool merutilm::rff::SettingsMenu::hasCheckbox(const int menuID) {
    if (const auto id = getIndex(menuID);
        checkIndex(id)
    ) {
...
void merutilm::rff::SettingsMenu::setCurrentActiveSettingsWindow(std::unique_ptr<SettingsWindow> &&scene) {
    currentActiveSettingsWindow = std::move(scene);
}


bool *merutilm::rff::SettingsMenu::getBool(RenderScene &scene, const int menuID) const {
    if (const auto id = getIndex(menuID);
        checkIndex(id)
    ) {
...
int merutilm::rff::SettingsMenu::getIndex(const int menuID) {
    return menuID - Constants::Win32::ID_MENUS;
}

bool merutilm::rff::SettingsMenu::checkIndex(const int index) const {
