...
RFFVideoWindow::RFFVideoWindow(const uint16_t width, const uint16_t height) : scene(RFFVideoRenderScene()) {
    ...
    renderWindow = CreateWindowEx(0, RFF::Win32::CLASS_VIDEO_RENDER_WINDOW, nullptr,
...
LRESULT RFFVideoWindow::videoWindowProc(const HWND hwnd, const UINT message, const WPARAM wParam,
                                        const LPARAM lParam) {
    const auto &window = *reinterpret_cast<RFFVideoWindow *>(GetWindowLongPtr(hwnd, GWLP_USERDATA));
    switch (message) {
        case WM_DESTROY: {
...
void RFFVideoWindow::createVideo(const Settings &settings,
    ...
    auto window = RFFVideoWindow(cw, ch);
    std::jthread thread(
        [&window, &cw, &ch, &imgWidth, &imgHeight, &writer, &settings, &open, &save] {
            ...
            const uint32_t maxNumber = IOUtilities::fileNameCount(open, RFF::Extension::MAP);
...
void RFFVideoWindow::messageLoop() const {
    MSG msg;
