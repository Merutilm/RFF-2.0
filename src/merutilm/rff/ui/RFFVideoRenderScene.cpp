...
RFFVideoRenderScene::RFFVideoRenderScene() : RFFScene() {
...
void RFFVideoRenderScene::applyColor(const Settings &settings) const {
...
void RFFVideoRenderScene::configure(const HWND wnd, const HDC hdc, const HGLRC context) {
...
float RFFVideoRenderScene::calculateZoom(const double defaultZoomIncrement) const {
    if (currentFrame < 1) {
...
GLMultipassRenderer &RFFVideoRenderScene::getRenderer() const{
    return *renderer;
}
...
void RFFVideoRenderScene::renderGL() {
...
const cv::Mat &RFFVideoRenderScene::getCurrentImage() const {
    return currentImage;
}