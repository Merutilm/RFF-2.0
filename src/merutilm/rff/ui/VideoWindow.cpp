...
merutilm::rff::VideoWindow::VideoWindow(const uint16_t width, const uint16_t height) : scene(VideoRenderScene()) {
...
merutilm::rff::VideoWindow::~VideoWindow() {
    ReleaseDC(renderWindow, hdc);
    PostMessage(videoWindow, WM_CLOSE, 0, 0);
}


void merutilm::rff::VideoWindow::setClientSize(const int width, const int height) const {
...
LRESULT merutilm::rff::VideoWindow::videoWindowProc(const HWND hwnd, const UINT message, const WPARAM wParam,
                                        const LPARAM lParam) {
...
void merutilm::rff::VideoWindow::createVideo(const Settings &settings,
    ...
    std::jthread thread(
        [&window, &cw, &ch, &imgWidth, &imgHeight, &writer, &settings, &open, &save] {
            GLLoader::createContext(window.hdc, &window.context);
...
void merutilm::rff::VideoWindow::messageLoop() const {
    MSG msg;
