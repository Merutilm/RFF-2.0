...
merutilm::rff::VideoRenderScene::VideoRenderScene() : GLScene() {
...
void merutilm::rff::VideoRenderScene::applyCurrentFrame() const {
    rendererIteration2Map->setCurrentFrame(currentFrame);
}

void merutilm::rff::VideoRenderScene::applyCurrentMap() const {
...
void merutilm::rff::VideoRenderScene::applyColor(const Settings &settings) const {
...
void merutilm::rff::VideoRenderScene::configure(const HWND wnd, const HDC hdc, const HGLRC context) {
...
float merutilm::rff::VideoRenderScene::calculateZoom(const double defaultZoomIncrement) const {
    if (currentFrame < 1) {
...
merutilm::rff::GLMultipassRenderer &merutilm::rff::VideoRenderScene::getRenderer() const{
    return *renderer;
}

void merutilm::rff::VideoRenderScene::setCurrentFrame(const float currentFrame) {
    this->currentFrame = currentFrame;
}

void merutilm::rff::VideoRenderScene::setMap(RFFMap &&normal, RFFMap &&zoomed) {
    this->normal = std::move(normal);
    this->zoomed = std::move(zoomed);
}

void merutilm::rff::VideoRenderScene::reloadImageBuffer(const uint16_t w, const uint16_t h) {
    pixels = std::vector<char>(static_cast<uint32_t>(w) * h * 3);
    currentImage = cv::Mat(h, w, CV_8UC3, pixels.data());
}


void merutilm::rff::VideoRenderScene::renderGL() {
...
const cv::Mat &merutilm::rff::VideoRenderScene::getCurrentImage() const {
    return currentImage;
}