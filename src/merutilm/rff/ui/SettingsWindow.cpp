...
merutilm::rff::SettingsWindow::SettingsWindow(const std::string_view &name) {
...
int merutilm::rff::SettingsWindow::getFixedNameWidth() const {
    RECT rect;
    GetClientRect(window, &rect);
    return (rect.right - rect.left) / Constants::Win32::SETTINGS_LABEL_WIDTH_DIVISOR;
}

int merutilm::rff::SettingsWindow::getFixedValueWidth() const {
    RECT rect;
    GetClientRect(window, &rect);
...
void merutilm::rff::SettingsWindow::adjustWindowHeight() const {
    RECT rect = {
...
int merutilm::rff::SettingsWindow::getIndex(const HWND wnd) {
    return GetDlgCtrlID(wnd) - Constants::Win32::ID_OPTIONS & 0x00ff;
}

int merutilm::rff::SettingsWindow::getRadioIndex(const HWND wnd) {
    const int offset = GetDlgCtrlID(wnd) - Constants::Win32::ID_OPTIONS;
    return offset / Constants::Win32::ID_OPTIONS_RADIO;
}

bool merutilm::rff::SettingsWindow::checkIndex(const int index) const {
    return index >= 0 && index < callbacks.size();
}

int merutilm::rff::SettingsWindow::getYOffset() const {
...
void merutilm::rff::SettingsWindow::createLabel(const std::string_view &settingsName, const std::string_view descriptionTitle,
...
LRESULT merutilm::rff::SettingsWindow::settingsWindowProc(const HWND window, const UINT message, const WPARAM wParam,
                                        const LPARAM lParam) {
...
LRESULT merutilm::rff::SettingsWindow::textFieldProc(const HWND window, const UINT message, const WPARAM wParam,
                                         const LPARAM lParam,
...
void merutilm::rff::SettingsWindow::callError(const int index) {
    error[index] = true;
...
std::string merutilm::rff::SettingsWindow::currValueToString(const int index) const {
    return (*unparsers[index])(references[index]);
}


void merutilm::rff::SettingsWindow::setWindowCloseFunction(std::function<void()> &&function) {
