...
namespace merutilm::vkh {
    void BufferImageUtils::initImage(Core &core, const ImageInitInfo &iii, VkImage *image, VkDeviceMemory *imageMemory,
        ...
        initImage(core.getLogicalDevice().getLogicalDeviceHandle(),
                  core.getPhysicalDeviceLoader().getPhysicalDeviceMemoryProperties(), iii, image, imageMemory,
                  imageView, mipmappedImageView, capacity);
    }

    void BufferImageUtils::initImage(const VkDevice device, const VkPhysicalDeviceMemoryProperties &memProperties,
                                     const ImageInitInfo &iii, VkImage *image, VkDeviceMemory *imageMemory,
                                     VkImageView *imageView, VkImageView *mipmappedImageView, VkDeviceSize *capacity) {
        const uint32_t mipLevels = genMipLevels(iii);
        createImage(device, iii, mipLevels, image);
        allocateImageMemory(device, memProperties, iii.properties, *image, imageMemory, capacity);
    ...
    void BufferImageUtils::createImage(const VkDevice device, const ImageInitInfo &iii, const uint32_t mipLevels,
                                       VkImage *image) {
    ...
    void BufferImageUtils::allocateImageMemory(const VkDevice device,
        ...
        *capacity = memRequirements.size;
        allocateMemory(device, memProperties, memRequirements, properties, imageMemory);
    }

    void BufferImageUtils::allocateMemory(const VkDevice device, const VkPhysicalDeviceMemoryProperties &memProperties,
                                          const VkMemoryRequirements &memRequirements,
                                          const VkMemoryPropertyFlags properties, VkDeviceMemory *memory) {
        VkMemoryAllocateInfo allocInfo = {
                .sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
                .pNext = nullptr,
                .allocationSize = memRequirements.size,
                .memoryTypeIndex = findMemoryTypeIndex(memProperties, memRequirements.memoryTypeBits, properties),
        };
    ...
    void BufferImageUtils::createImageView(const VkDevice device, const VkImage image,
                                           const VkImageViewType imageViewType, const VkFormat imageFormat,
        ...
        const VkImageViewCreateInfo viewInfo = {.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO,
                                                ...
                                                .subresourceRange = {.aspectMask = getAspectMask(imageFormat),
                                                                     .baseMipLevel = 0,
                                                                     .levelCount = 1,
                                                                     .baseArrayLayer = 0,
                                                                     .layerCount = 1}};
    ...
    void BufferImageUtils::createMipmappedImageView(const VkDevice device, const VkImage image,
                                                    const VkImageViewType imageViewType, const VkFormat imageFormat,
                                                    const uint32_t mipLevels, VkImageView *mipmappedImageView) {
        const VkImageViewCreateInfo viewInfo = {.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO,
                                                .pNext = nullptr,
                                                .flags = 0,
                                                .image = image,
    ...
    void BufferImageUtils::initBuffer(const VkDevice device, const VkPhysicalDeviceMemoryProperties &memProperties,
                                      const BufferInitInfo &bii, VkBuffer *buffer, VkDeviceMemory *bufferMemory,
                                      VkDeviceSize *allocationSize) {
        createBuffer(device, bii.size, bii.usage, buffer);
        allocateBufferMemory(device, memProperties, bii.properties, *buffer, bufferMemory, allocationSize);
        vkBindBufferMemory(device, *buffer, *bufferMemory, 0);
    }

    void BufferImageUtils::createBuffer(const VkDevice device, const VkDeviceSize size, const VkBufferUsageFlags usage,
                                        VkBuffer *buffer) {
    ...
    void BufferImageUtils::allocateBufferMemory(const VkDevice device,
        ...
        allocateMemory(device, memProperties, memRequirements, properties, bufferMemory);
        *allocationSize = memRequirements.size;
    }
